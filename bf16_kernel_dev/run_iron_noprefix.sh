...
def pt_design(a: In, b: Out, *, N: CompileTime[int]):
    ...
    # Direct ExternalFunction — NO symbol_prefix, NO hash prefix
    kp = ExternalFunction(
        "passThroughLine",
    ...
    print(f"Kernel name: {kp._name}", flush=True)
    print(f"Symbol prefix: {kp._symbol_prefix}", flush=True)
...
print('Running IRON passthrough (Peano, no prefix)...', flush=True)
result = pt_design(a, b, N=N)
print('Success! First 8:', np.array(b)[:8], flush=True)
PYEOF
