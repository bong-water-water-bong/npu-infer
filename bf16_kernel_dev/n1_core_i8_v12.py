#!/usr/bin/env python3
#
# INT8 MLIR generator v12 — SINGLE CORE PER COMPUTE GROUP
# - Only 1 core (no K-slice interleaving issue)
# - M-tiling: each M-tile of 32 rows goes to 1 core
# - K-loop inside core correctly accumulates all K-values
# - N-tiling: each N-block of 128 cols goes to 1 core
# - Each core writes its own (32×128) output tile
import argparse
import numpy as np
...
def my_matmul(M, K, N, m, k, n):
    n_aie_cols = 8  # 8 cores, each handles one (M=32, N=128) output tile
    n_aie_rows = 1
    dtype_in = np.int8
    dtype_out = np.int16
    mtk = 512  # K per L2 element
    
    @device(AIEDevice.npu2)
    def device_body():
        A_l2_ty = np.ndarray[(m, mtk), np.dtype[dtype_in]]
        A_l1_ty = np.ndarray[(m // 4, k), np.dtype[dtype_in]]  # 8x64, m//4 because... wait
        # Actually for scalar kernel we need m x k = 32x64
        ...
        # Per-core (single-consumer) A and B fifos
        A_s = [None]*n_aie_cols
        A_c = [None]*n_aie_cols
        B_s = [None]*n_aie_cols
        B_c = [None]*n_aie_cols
        ...
        for c in range(n_aie_cols):
            # A: shim[0] → mem[c] → core[c] — single consumer
            ...
            # B: shim[c] → mem[c] → core[c] — single consumer
        ...
        # Core body: each core handles ONE M-tile, ONE N-block
        # K-loop inside core for correct accumulation
        for c in range(n_aie_cols):
            @core(ct[c])
            def core_body():
