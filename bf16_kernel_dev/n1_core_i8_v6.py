#!/usr/bin/env python3
#
# INT8 MLIR generator v6 — EACH core has its own A fifo (broadcast workaround)
# - 8 separate A fifos, all reading from same shim[0] and same L3 offset
# - 8 separate B fifos, each from shim[c] for N-block
# - Original B tap covering full K-span (auto-distributes K-slices)
# - All 8 cores start A at the same K-offset
import argparse
import numpy as np
...
def my_matmul(M, K, N, m, k, n):
    ...
    def device_body():
        ...
        st, mt, ct = tiles[0], tiles[1], tiles[2:][0]
        
        # A: per-core fifo, all from shim[0] (tap handles unique offset per core)
        A_s = [None]*n_aie_cols
        A_c = [None]*n_aie_cols
        for c in range(n_aie_cols):
            A_s[c] = object_fifo(f"A_S{c}", st[c], mt[c], 2, A_l2_ty, None,
                [[(m, k), (mtk // k, m * k), (k, 1)]])
            A_c[c] = object_fifo(f"A_C{c}", mt[c], ct[c], 2, A_l1_ty)
        ...
        for c in range(n_aie_cols):
            ...
            B_c[c] = object_fifo(f"B_C{c}", mt[c], ct[c], 2, B_l1_ty)
        ...
        C_s = [None]*n_aie_cols
        for c in range(n_aie_cols):
            C_c[c] = object_fifo(f"C_c{c}", ct[c], mt[c], 1, C_l1_ty)
        ...
        def seq(A, B, C):
            ...
            for gi in range(n_cg):
                ...
                # A: each core gets its own K-tile from the L2 stride
                for c in range(n_aie_cols):
