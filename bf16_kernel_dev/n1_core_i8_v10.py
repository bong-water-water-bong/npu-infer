#!/usr/bin/env python3
#
# INT8 MLIR generator v10 — PER-CORE A FIFOS with mtk=128
# - Minimal memory footprint per mem tile
import argparse
import numpy as np
...
def my_matmul(M, K, N, m, k, n):
    ...
    dtype_in = np.int8
    dtype_out = np.int16
    mtk = 128  # Minimal — 4KB per L2 element
    
    @device(AIEDevice.npu2)
    def device_body():
        A_l2_ty = np.ndarray[(m, mtk), np.dtype[dtype_in]]  # 32×128 = 4KB
        ...
        for c in range(n_aie_cols):
            A_s[c] = object_fifo(f"AS{c}", st[0], mt[c], 1, A_l2_ty)  # depth 1!
            A_c[c] = object_fifo(f"AC{c}", mt[c], ct[c], 1, A_l1_ty)  # depth 1!
            object_fifo_link(A_s[c], A_c[c])
            B_s[c] = object_fifo(f"BS{c}", st[c], mt[c], 1, B_l2_ty)  # depth 1!
            B_c[c] = object_fifo(f"BC{c}", mt[c], ct[c], 1, B_l1_ty)  # depth 1!
            object_fifo_link(B_s[c], B_c[c])
            C_c_f[c] = object_fifo(f"CC{c}", ct[c], mt[c], 1, C_l1_ty)
            C_s_f[c] = object_fifo(f"CS{c}", mt[c], st[c], 1, C_l2_ty)
        ...
        def seq(A, B, C):
            ...
            for gi in range(n_cg):
                ...
                c_base = gi * n_aie_cols
                for c in range(n_aie_cols):
                    ct_ = shim_dma_single_bd_task(C_s_f[c], C, tap=C_taps[c_base + c], issue_token=True)
