...
int main(){
    ...
    for(int step=0;step<8;step++){
	    ...
	    float mx=logits[0]; for(int i=1;i<V;i++)if(logits[i]>mx)mx=logits[i]; double sum=0; for(int i=0;i<V;i++){logits[i]=expf(logits[i]-mx); sum+=logits[i];} float r=(float)rand()/RAND_MAX*sum,acc=0; int token=0; for(int i=0;i<V;i++){acc+=logits[i];if(acc>=r){token=i;break;}}
