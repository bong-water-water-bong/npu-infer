...
struct Npu{
    ...
    std::vector<uint32_t>ins;std::unique_ptr<xrt::bo>bI,bA,bB,bC;std::vector<float>Af,Cf;uint16_t*Ab=nullptr;
    bool init(){
        ...
        Af.resize(XM*XK); Cf.resize(XM*XN); Ab=(uint16_t*)bA->map();
    }
    void upload_A(const float*A,int M,int K){memset(Af.data(),0,XM*XK*sizeof(float));for(int m=0;m<M;m++)for(int k=0;k<K;k++)Af[m*XK+k]=A[m*K+k];auto As=gemm_atb::layout_A_L1_2x1_8x8block(Af,XM,XK,M_TILE,K_TILE);for(int i=0;i<XM*XK;i++)Ab[i]=fbf16(As[i]);bA->sync(XCL_BO_SYNC_BO_TO_DEVICE);}
    void exec_read(const uint8_t*pB,size_t psz,int out_feat,float*C,int M){memcpy(bB->map(),pB,psz);bB->sync(XCL_BO_SYNC_BO_TO_DEVICE);memset(bC->map(),0,(size_t)XM*XN*2);bC->sync(XCL_BO_SYNC_BO_TO_DEVICE);auto run=(*k)((unsigned)3,*bI,ins.size(),*bA,*bB,*bC);run.wait();bC->sync(XCL_BO_SYNC_BO_FROM_DEVICE);uint16_t*Cb=(uint16_t*)bC->map();for(int i=0;i<XM*XN;i++)Cf[i]=bf16f(Cb[i]);auto Cu=gemm_atb::layout_inverse_C_L1_2x2_8x8block(Cf,XM,XN,NROWS*M_TILE,N_TILE);for(int m=0;m<M;m++)for(int n=0;n<out_feat;n++)C[m*out_feat+n]=Cu[m*XN+n];}
    }
};
