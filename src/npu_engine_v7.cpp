/**
 * NPU Engine v7 — BFP16 1024×1024 xclbin. Working, validated, scale=1.0
 */
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
static const char*XCLBIN="/home/bcloud/npu-sandbox/npu-infer/bf16_kernel_dev/build/design_bf16_v3.xclbin";
static const char*INSTRS="/home/bcloud/npu-sandbox/npu-infer/bf16_kernel_dev/build/design_bf16_v3.insts";
...
static std::vector<uint8_t> pack_bfp16(const float*w,int out_feat,int in_feat,int deq_rows){
...
int main(){
    setvbuf(stdout,NULL,_IONBF,0);printf("=== NPU Engine v7 — BFP16 (scale=1.0) Working ===\n\n");
    ...
    printf("Pre-dequantize + pack...\n");auto t0=std::chrono::steady_clock::now();
    PW pw[28];float*lm_f;int lm_r,lm_c;
    for(int l=0;l<L;l++){
        int unused;
        float*qw=dequant_i8_to_float(i8p(lo[l].qp),256,&lm_r,&unused);pw[l].q=pack_bfp16(qw,lm_r,H,lm_r);free(qw);
        float*kw=dequant_i8_to_float(i8p(lo[l].kp),128,&lm_r,&unused);pw[l].k=pack_bfp16(kw,lm_r,H,lm_r);free(kw);
        float*vw=dequant_i8_to_float(i8p(lo[l].vp),128,&lm_r,&unused);pw[l].v=pack_bfp16(vw,lm_r,H,lm_r);free(vw);
        float*ow=dequant_i8_to_float(i8p(lo[l].op),256,&lm_r,&unused);pw[l].o=pack_bfp16(ow,H,NH*HD,lm_r);free(ow);
        float*gw=dequant_i8_to_float(i8p(lo[l].gp),384,&lm_r,&unused);pw[l].g=pack_bfp16(gw,lm_r,H,lm_r);free(gw);
        float*uw=dequant_i8_to_float(i8p(lo[l].up),384,&lm_r,&unused);pw[l].u=pack_bfp16(uw,lm_r,H,lm_r);free(uw);
        float*dw=dequant_i8_to_float(i8p(lo[l].dp),384,&lm_r,&unused);pw[l].d=pack_bfp16(dw,H,IM,lm_r);free(dw);
        if(l%7==0)printf("  L%d\n",l);
    }
    ...
    int n_prompt=9,seq_pos=0;
    for(int pi=0;pi<n_prompt;pi++){int tok=prompt_tokens[pi];
    ...
    for(int step=0;step<8;step++){
        ...
        memcpy(no.data(),h.data(),H*4);
        rms_norm(no.data(),fnw,H,eps);
        for(int n=0;n<V;n++){double s=0;for(int k=0;k<H;k++){uint16_t r=emb[n*H+k];if((r&0x7F80)!=0x7F80)s+=no[k]*bf16f(r);}logits[n]=(float)s;}
        float mx=logits[0],mn=logits[0];for(int i=1;i<V;i++){if(logits[i]>mx)mx=logits[i];if(logits[i]<mn)mn=logits[i];}
        double sum=0;for(int i=0;i<V;i++){logits[i]=expf(logits[i]-mx);sum+=logits[i];}
        float r=(float)rand()/RAND_MAX*sum,acc=0;int token=0;for(int i=0;i<V;i++){acc+=logits[i];if(acc>=r){token=i;break;}}
